(function () {
	...
	if (fs.existsSync('server.key') && fs.existsSync('server.crt')) {
		...
		server.listen(8080);
		console.log("Noname WSS (Secure) server started on port 8080");
	} else {
		wss = new WebSocketServer({ port: 8080 });
		console.log("Noname WS (Insecure) server started on port 8080");
	}
	...
	wss.on("connection", function (ws, req) {
		console.log("New connection request from: " + req.socket.remoteAddress + " URL: " + req.url);
		// 添加简单的 Token 认证
		// 如果环境变量中设置了 SERVER_TOKEN，则检查 URL 参数
		if (process.env.SERVER_TOKEN) {
			const url = req.url || "";
			// 简单的字符串查找，或者解析 url
			// 假设连接地址格式为 ws://host:port/?token=xxx
			if (url.indexOf("token=" + process.env.SERVER_TOKEN) === -1) {
				console.log("Connection rejected: Invalid token from " + req.socket.remoteAddress);
				ws.close();
				return;
			}
		}
